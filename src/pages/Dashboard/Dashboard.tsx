...
export const SchoolDashboard: React.FC = () => {
  ...
  const totalFeesCollected = students.reduce(
    (acc, s) => acc + (s.feesPaid || 0),
    0,
  );
  const totalFeesPending = students.reduce(
    (acc, s) =>
      acc +
      (s.remainingFees ?? Math.max(0, (s.totalFees || 0) - (s.feesPaid || 0))),
    0,
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        const paid = record.feesPaid || 0;
        const remaining = record.remainingFees ?? Math.max(0, total - paid);
        if (total === 0)
        ...
        return (
          <div>
            ...
            {remaining > 0 ? (
              <span style={{ fontSize: 11, color: "#ffa552", fontWeight: 500 }}>
                Due: ₹{remaining.toLocaleString()}
              </span>
            ) : (
              <span style={{ fontSize: 11, color: "#45a29e", fontWeight: 500 }}>
                Fully Paid
              </span>
            )}
          </div>
    ...
    <div style={{ position: "relative" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <Paragraph style={{ color: "var(--text-secondary)", marginTop: 4 }}>
          Live metrics tracking student enrollment states, fee balances, verify
          lists, and academic units.
        </Paragraph>
      </div>
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            title={
              ...
              >
                <UserOutlined style={{ color: "#45a29e" }} /> Total Directory
              </span>
            }
            value={totalStudents}
            suffix={
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            title={
              ...
              >
                <HourglassOutlined style={{ color: "#ffa552" }} /> Onboarding
                Queue
              </span>
            }
            value={pendingCount}
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            title={
              ...
              >
                <CheckCircleOutlined style={{ color: "#45a29e" }} /> Fees
                Collected
              </span>
            }
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            title={
              ...
              >
                <BookOutlined style={{ color: "#ffa552" }} /> Fees Remaining
              </span>
            }
